...
# Setup constant values for screen resolution
# Can be changed to suit screen size
SCREEN_WIDTH = 1280
SCREEN_HEIGHT = 718
...
def mainGame():
    ...
    class hitButton(pygame.sprite.Sprite):
        ...
        def __init__(self, positionXIn, positionYIn):
            pygame.sprite.Sprite.__init__(self)
            self.image, self.rect = imageLoad("hit-grey.png", 2)
        ...
        def update(self, mX, mY, deck, deadDeck, playerHand, cards, pCardPos, roundEnd, cardSprite, click):
            ...
            if roundEnd == 0: self.image, self.rect = imageLoad("hit.png", 2)
            else: self.image, self.rect = imageLoad("hit-grey.png", 2)
    ...
    class standButton(pygame.sprite.Sprite):
        ...
        def __init__(self, positionXIn, positionYIn):
            pygame.sprite.Sprite.__init__(self)
            self.image, self.rect = imageLoad("stand-grey.png", BUTTON_IMAGE)
            self.position = (positionXIn, positionYIn)
            self.xPos = positionXIn
            self.yPos = positionYIn
        ...
        def update(self, mX, mY, deck, deadDeck, playerHand, dealerHand, cards, pCardPos, roundEnd, cardSprite, funds, bet, displayFont):
            ...
            if roundEnd == 0: self.image, self.rect = imageLoad("stand.png", BUTTON_IMAGE)
            else: self.image, self.rect = imageLoad("stand-grey.png", BUTTON_IMAGE)
    ...
    class doubleButton(pygame.sprite.Sprite):
        ...
        def __init__(self, positionXIn, positionYIn):
            pygame.sprite.Sprite.__init__(self)
            self.image, self.rect = imageLoad("double-grey.png", BUTTON_IMAGE)
            self.position = (positionXIn, positionYIn)
            self.xPos = positionXIn
            self.yPos = positionYIn
        ...
        def update(self, mX, mY,   deck, deadDeck, playerHand, dealerHand, playerCards, cards, pCardPos, roundEnd, cardSprite, funds, bet, displayFont):
            ...
            if roundEnd == 0 and funds >= bet * 2 and len(playerHand) == 2: self.image, self.rect = imageLoad("double.png", BUTTON_IMAGE)
            else: self.image, self.rect = imageLoad("double-grey.png", BUTTON_IMAGE)
    ...
    class dealButton(pygame.sprite.Sprite):
        ...
        def __init__(self, positionXIn, positionYIn):
            pygame.sprite.Sprite.__init__(self)
            self.image, self.rect = imageLoad("deal.png", BUTTON_IMAGE)
            self.position = (positionXIn, positionYIn)
            self.xPos = positionXIn
            self.yPos = positionYIn
        ...
        def update(self, mX, mY, deck, deadDeck, roundEnd, cardSprite, cards, playerHand, dealerHand, dCardPos, pCardPos, displayFont, playerCards, click, handsPlayed):
            ...
            if roundEnd == 1: self.image, self.rect = imageLoad("deal.png", BUTTON_IMAGE)
            else: self.image, self.rect = imageLoad("deal-grey.png", BUTTON_IMAGE)
            ...
            if self.rect.collidepoint(mX, mY) == 1:
                if roundEnd == 1 and click == 1:
                    ...
                    dCardPos = (100, 150) # Deal card start position 
                    pCardPos = (self.xPos-150, self.yPos)
                    #pCardPos = (1000,550)
    ...
    class betButtonUp(pygame.sprite.Sprite):
        ...
        def __init__(self, positionXIn, positionYIn):
            pygame.sprite.Sprite.__init__(self)
            self.image, self.rect = imageLoad("up.png", BUTTON_IMAGE)
            self.position = (positionXIn, positionYIn)
            self.xPos = positionXIn
            self.yPos = positionYIn
            
        def update(self, mX, mY, bet, funds, click, roundEnd):
            if roundEnd == 1: self.image, self.rect = imageLoad("up.png", BUTTON_IMAGE)
            else: self.image, self.rect = imageLoad("up-grey.png", BUTTON_IMAGE)
    ...
    class betButtonDown(pygame.sprite.Sprite):
        ...
        def __init__(self, positionXIn, positionYIn):
            pygame.sprite.Sprite.__init__(self)
            self.image, self.rect = imageLoad("down.png", BUTTON_IMAGE)
            self.position = (positionXIn, positionYIn)
            self.xPos = positionXIn
            self.yPos = positionYIn
            
        def update(self, mX, mY, bet, click, roundEnd):  
            if roundEnd == 1: self.image, self.rect = imageLoad("down.png", BUTTON_IMAGE)
            else: self.image, self.rect = imageLoad("down-grey.png", BUTTON_IMAGE)
    ...
    class quitButton(pygame.sprite.Sprite):
        ...
        def __init__(self, positionXIn, positionYIn):
            pygame.sprite.Sprite.__init__(self)
            self.image, self.rect = imageLoad("quit.png", BUTTON_IMAGE)
            self.position = (positionXIn, positionYIn)
            self.xPos = positionXIn
            self.yPos = positionYIn
    ...
    background, backgroundRect = imageLoad("bjs.jpg", BACKGROUND_IMAGE)
    ...
    # Inititalise button position class / structure
    uiButtonPosition = buttonPositioningValues()

    # This creates instances of all the button sprites
    bbU = betButtonUp(uiButtonPosition.upArrowButtonPosition_X, uiButtonPosition.upArrowButtonPosition_Y)
    bbD = betButtonDown(uiButtonPosition.downArrowButtonPosition_X, uiButtonPosition.downArrowButtonPosition_Y)
    standButton = standButton(uiButtonPosition.standButtonPosition_X, uiButtonPosition.standButtonPosition_Y)
    dealButton = dealButton(uiButtonPosition.dealButtonPosition_X, uiButtonPosition.dealButtonPosition_Y)
    hitButton = hitButton(uiButtonPosition.hitButtonPosition_X, uiButtonPosition.hitButtonPosition_Y)
    doubleButton = doubleButton(uiButtonPosition.doubleButtonPosition_X, uiButtonPosition.doubleButtonPosition_Y)
    
    qButton = quitButton(uiButtonPosition.quitButtonPosition_X, uiButtonPosition.quitButtonPosition_Y)
    
    # This group containts the button sprites
    buttons = pygame.sprite.Group(dealButton, hitButton, standButton, doubleButton, bbU, bbD,qButton)
    ...
    while 1:
        ...
        screen.blit(displayFont, (10,uiButtonPosition.dealButtonPosition_Y+170))
        fundsFont = pygame.font.Font.render(textFont, "Funds: $%.2f" %(funds), 1, (255,255,255), (0,0,0))
        screen.blit(fundsFont, (uiButtonPosition.upArrowButtonPosition_X-25,50))
        betFont = pygame.font.Font.render(textFont, "Bet: $%.2f" %(bet), 1, (255,255,255), (0,0,0))
        screen.blit(betFont, (uiButtonPosition.upArrowButtonPosition_X-25,100))
        hpFont = pygame.font.Font.render(textFont, "Round: %i " %(handsPlayed), 1, (255,255,255), (0,0,0))
        screen.blit(hpFont, (uiButtonPosition.upArrowButtonPosition_X-25, 75))
        ...
        isQuit = qButton.update(mX, mY, click)
        if isQuit == 0:
            pygame.quit()
            return 0
